    ...
    for batch_idx, inputs in enumerate(train_loader):
        ...
        if fabric.is_global_zero:
            ...
            if step % cfg.run.save_frequency == 0 and step != 0:
                trainer.model.save_model(optimiser, step, lr_scheduler, ema)
            # save the validation results
            early_phase = (step < 6000) and (step % 500 == 0)
            if (early_phase or step % cfg.run.val_frequency == 0) and step != 0: # and step != 0:
...
def main(cfg: DictConfig):
    ...
    if cfg.run.checkpoint:
        ...
        loaded_optim, loaded_step, loaded_scheduler = model.load_model(cfg.run.checkpoint, optimiser=optimiser, lr_scheduler=lr_scheduler)
        print("===== Exchange optimizer/step/scheduler to loaded one =====")
        # optimiser = loaded_optim
        lr_scheduler = loaded_scheduler
        trainer.step = loaded_step
    else:
